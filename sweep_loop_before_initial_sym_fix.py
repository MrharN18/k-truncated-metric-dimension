...
def frontier_group_clean(graph, dist, current_full, rel, current_rel, persistent=None):
    ...
    if distant_used:
        ...
        # The first uncovered vertex may be the unique distant vertex.
        # If it is the only uncovered vertex left, no further frontier is
        # needed. Otherwise keep the ORIGINAL frontier based on the second
        # uncovered vertex, but also include the first uncovered vertex itself
        # as an optional resolver position.
        if len(uncovered) == 1:
            return None

        first_rel, first = uncovered[0]
        y_rel, y = uncovered[1]
        end_rel = min(y_rel + k, n - 1)
        cand = {
            u for u in range(n)
...
def clean_candidate_feasible(graph, dist, persistent, pos, current_full, rel, current_groups=None):
    ...
    the current frontier.  OR-like groups may be deferred naturally: if the
    candidate does not hit them, they remain feasible exactly when at least
    one candidate is still at/after the proposed position.

    A temporary frontier is mandatory for the current free arc.  Hitting it
    ...
    for g in blockers:
        ...
        if group_contains(g, pos):
            ...
            # This is also the generic OR-deferral rule: an untouched OR may
            # be postponed if and only if at least one of its candidates
            # remains available without backtracking.
            if not any(rel(v) >= pos_rel
                       for b in g['candidates'] for v in b):
                return False
    return True
...
def _is_deferrable_or_group(group):
    """Whether an active group may be deferred past its early candidates.

    Symmetry groups and explicit multi-branch OR groups are disjunctive:
    they need one later hit, not necessarily the earliest available one.
    """
    if group.get('source', (None,))[0] == 'symmetry_clean':
        return True
    return len(group.get('candidates', [])) > 1


def _clean_candidate_choices(groups, rel):
    """Candidate points from overlap chains, with one-step OR deferral.

    Normally candidates come from the earliest overlap chain.

    If the earliest group is OR-like and a later non-overlapping group starts
    before the last still-available candidate of that OR group, also expose the
    later group's overlap-chain candidates.  Feasibility then decides whether
    the early OR can safely be deferred, while the ordinary greedy comparison
    ...
    def add_chain_from(start_idx):
        for b0 in ordered[start_idx]['candidates']:
            cur = set(b0)
            if not cur:
                continue
            for pos in cur:
    ...
    # Standard earliest overlap chain.
    add_chain_from(0)

    # Special OR deferral: if the earliest group is disjunctive, find the
    # first later group that does not overlap it.  If the OR still has a
    # candidate strictly after that later group's first position, then the
    # early part of the OR may be skipped and that later group may be processed
    # first.  Expose that later overlap chain as an alternative.
    first = ordered[0]
    if _is_deferrable_or_group(first):
        first_union = set().union(*first['candidates']) if first['candidates'] else set()
        if first_union:
            latest_first = max(rel(v) for v in first_union)
            for idx in range(1, len(ordered)):
                g = ordered[idx]
                g_union = set().union(*g['candidates']) if g['candidates'] else set()
                if not g_union:
                    continue
                if first_union & g_union:
                    continue
                earliest_g = min(rel(v) for v in g_union)
                if any(rel(v) > earliest_g for v in first_union):
                    add_chain_from(idx)
                break
...
def _has_or_deferral_choice(groups, rel):
    """Whether the current state has an early OR group that can be deferred."""
    if not groups:
        return False
    ordered = sorted(groups, key=lambda g: group_earliest(g, rel))
    first = ordered[0]
    if not _is_deferrable_or_group(first):
        return False

    first_union = set().union(*first['candidates']) if first.get('candidates') else set()
    if not first_union:
        return False

    for g in ordered[1:]:
        g_union = set().union(*g['candidates']) if g.get('candidates') else set()
        if not g_union:
            continue
        if first_union & g_union:
            continue
        earliest_g = min(rel(v) for v in g_union)
        return any(rel(v) > earliest_g for v in first_union)
    return False


def _next_feasible_position_score(graph, dist, future_groups,
                                  trial_persistent, trial_full, rel,
                                  after_rel):
    """How far ahead the next resolver can be placed.

    Returns -1 for an unresolved dead end with no future requirement,
    graph.n for a state that is already fully resolving, otherwise the
    furthest relative position among feasible candidates for the immediate
    next placement.
    """
    if is_k_truncated_resolving(graph, trial_full, dist):
        return graph.n
    ...
    best = -1
    for _depth, nxt in _clean_candidate_choices(forward, rel):
        if clean_candidate_feasible(
            graph, dist, trial_persistent, nxt, trial_full, rel,
            current_groups=future_groups
        ):
            best = max(best, rel(nxt))
    return best
...
def _try_clean_symmetric_moveback(graph, dist, persistent_before,
                                  current_groups, choice, previous_anchor,
                                  current_full_before, rel):
    """Port of the legacy symmetric-pair backward correction.

    The correction is attempted ONLY when ``choice`` creates a new symmetric
    pair.  Move backward toward ``previous_anchor`` while preserving every
    current requirement that the original choice satisfied.  A move is adopted
    only when it resolves the newly-created symmetric pair(s), creates no more
    immediate future requirements, and moves the next requirement strictly
    further forward.
    """
    n = graph.n
    trial_full = current_full_before | {choice}
    pairs = find_symmetric_pairs(
        graph, dist, choice, trial_full - {choice}
    )
    if not pairs:
        return None

    choice_rel = rel(choice)
    prev_rel = rel(previous_anchor)
    if choice_rel <= prev_rel + 1:
        return None

    # Moving backward must not abandon any CURRENT group that the chosen
    # vertex actually satisfied.
    hit_groups = [g for g in current_groups if group_contains(g, choice)]

    orig_persistent, orig_sym, orig_frontier, orig_future = _clean_future_state(
        graph, dist, persistent_before, choice, current_full_before, rel
    )
    next_orig = _clean_next_requirement_rel(
        orig_future, rel, choice_rel, n
    )

    # Search one step backward at a time, exactly as the legacy correction did.
    for moved_rel in range(choice_rel - 1, prev_rel, -1):
        moved = next(v for v in range(n) if rel(v) == moved_rel)

        if any(not group_contains(g, moved) for g in hit_groups):
            continue

        if not clean_candidate_feasible(
            graph, dist, persistent_before, moved, current_full_before, rel,
            current_groups=current_groups
        ):
            continue

        # The moved resolver itself must break every symmetric pair that the
        # original placement created.
        if any(
            k_trunc(dist[moved][u], graph.k)
            == k_trunc(dist[moved][up], graph.k)
            for (u, up) in pairs
        ):
            continue

        moved_persistent, moved_sym, moved_frontier, moved_future = _clean_future_state(
            graph, dist, persistent_before, moved, current_full_before, rel
        )

        next_moved = _clean_next_requirement_rel(
            moved_future, rel, moved_rel, n
        )

        adopt = next_moved > next_orig

        # Without second look-ahead, a tied next deadline does not justify
        # moving the resolver backward; keep the original furthest choice.
        ...
        if adopt:
            return {
                'position': moved,
                'persistent': moved_persistent,
                'symmetry': moved_sym,
                'frontier': moved_frontier,
                'next_original': next_orig,
                'next_moved': next_moved,
                'pairs': pairs,
            }
...
def sweep_from_clean_frontier(graph, dist, initial_groups, start_vertex,
    ...
    while steps < max_steps:
        ...
        # Consider only forward candidates.
        forward_groups = []
        for g in groups:
            branches = []
            for b in g['candidates']:
        ...
        for depth, pos in _clean_candidate_choices(forward_groups, rel):
            ...
            # Special OR-deferral comparison.  Keep the established OR
            # lookahead first, but replace the old "number of groups" burden
            # with the real next-requirement deadline.
            if _has_or_deferral_choice(forward_groups, rel):
                next_score = _next_feasible_position_score(
                    graph, dist, future_groups, trial_persistent,
                    trial_full, rel, rel(pos)
                )
                key = (next_score, depth, next_deadline, rel(pos))
            else:
                # Ordinary sweep: prefer greater current overlap, then the
                # later next deadline, and finally the furthest current feasible
                # position.
                key = (depth, next_deadline, rel(pos))
        ...
        # Symmetric-pair backward correction, ported from the legacy sweep.
        # The original furthest-feasible choice is kept unless moving it
        # backward resolves the symmetry AND shifts the next requirement
        # strictly further forward.
        move_info = _try_clean_symmetric_moveback(
            graph, dist, persistent, forward_groups, choice,
            current_anchor, current_full, rel
        )
        if move_info is not None:
            choice = move_info['position']
